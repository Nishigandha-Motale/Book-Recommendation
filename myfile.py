...
def pop_load_Image(df):
    return HTML(
        df[['Image-URL-M', 'Book-Title', 'Book-Author', 'Year-Of-Publication', 'Publisher']].to_html(
            formatters={'Image-URL-M': path_to_image_html}, escape=False))
...
if picked_userid not in pt.index:
    d = popular_df[popular_df['num_ratings'] >= 250].sort_values('avg_rating', ascending=False).head(t)
    d = d[['Image-URL-M', 'Book-Title', 'Book-Author', 'Year-Of-Publication', 'Publisher']]
    d = d.reset_index()
    rs = pop_load_Image(d)
    rs
    ...
    def load_Image(df):
        return HTML(
            df[['Image-URL-M', 'Book-Title', 'Book-Author', 'Year-Of-Publication', 'Publisher', 'Book-Rating']].to_html(
                formatters={'Image-URL-M': path_to_image_html}, escape=False))
